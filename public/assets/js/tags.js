...
class Tags {
  ...
  _createBadge(text, value = null, data = {}) {
    ...
    if (this._config.baseClass) {
      ...
      classes = [...classes, ...["bg-" + badgeStyle, "mw-100" , "overflow-x-hidden","pe-3"]];
    }/*  else {
    ...
    if (allowClear) {
      ...
      if (this._config.clearEnd) {
        ...
        btnMargin = bver === 5 ? "me-1" : "mr-2";
        btnFloat = bver === 5 ? "float-start" : "float:left;";
      }
      const btn =
        bver === 5
          ? '<button type="button" style="font-size:1.2em" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Remove" class="' +
            btnMargin +
            " " +
            btnFloat +
            " btn-close " +
